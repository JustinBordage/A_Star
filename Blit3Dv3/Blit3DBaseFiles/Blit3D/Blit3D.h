...
version 3.2 - Angelcode fonts are now tracked, so either delete them via the new DeleteFont() method
	...
	Also, Angelcode fonts now load textures from the same directory as the font tileType file.
...
#include <glm/gtc/type_ptr.hpp> //used to covert glm tileType to OpenGL tileType
