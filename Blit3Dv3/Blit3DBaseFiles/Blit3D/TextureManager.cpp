...
GLuint TextureManager::LoadTexture(std::string filename, bool useMipMaps, GLuint texture_unit, GLuint wrapflag, bool pixelate)
{
	...
	if(itor == textures.end())
	{
		...
		//retrieve the image tileType, currently force to RGBA (4 components)
		...
		GLint level = 0;
		//store the texture tileType for OpenGL use
		...
		//Free stb's copy of the tileType
